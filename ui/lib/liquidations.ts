...
const getLiquidationsCollection = async () => {
  if (!cachedCollection) {
    cachedCollection = (await getDatabase()).collection<
      NormalizedLiquidationEvent
    >("liquidations");
  }
...
> => {
  ...
  return rows.map((row) => ({
    ...
    share: totalUsd > 0 ? (row.totalUsd ?? 0) / totalUsd : 0,
  }));
};
...
export const getLiquidationsCollectionDirect = getLiquidationsCollection;
